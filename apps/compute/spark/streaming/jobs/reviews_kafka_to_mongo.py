...
def main() -> None:
    spark = create_spark_session(app_name="reviews-kafka-to-mongo")
    topic = os.getenv("KAFKA_TOPIC_REVIEW", "raw_data_review")
    collection = os.getenv("MONGO_COLLECTION_REVIEW", "reviews")

    reviews_stream = read_kafka_json(spark, topic, review_schema())
    query = start_stream_to_mongo(
        reviews_stream,
        collection=collection,
        checkpoint_env="KAFKA_REVIEWS_CHECKPOINT",
        default_checkpoint="/tmp/reviews_checkpoint",
    )
